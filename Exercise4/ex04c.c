...
void demo(void *arg)
{
  ...
  int num=*(int *)arg;
  int newMidPrio = MID + 10;
  int newLowPrio = LOW + 10;
  printf("Task  : %d\n",num);
  ...
  while(runtime < EXECTIME) {
    ...
    if ((num == 2) && (runtime ==  EXECTIME/2)) {
      rt_task_set_priority(&demo_task[1], newMidPrio);
      rt_task_set_priority(&demo_task[0], newLowPrio);
    }
...
void startup()
{
  ...
  printf("Assigning task priority...... \n");
